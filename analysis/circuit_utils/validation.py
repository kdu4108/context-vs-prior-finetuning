...
from utils import format_prompts, PROMPTS_DICT
...
def preprocess_dataset(df, tokenizer, MODEL_TYPE):
    ...
    for i, row in df.iterrows():
        ...
        out["counterfactual"] = answers.loc[row.context]["context"] if row.weight_context == 0.0 else answers.loc[row.context]["prior"]
        data.append(out)
...
def get_true_false_ids(df, tokenizer):
    ...
    counterfactuals = [el[2] for el in counterfactuals] # first two tokens are "\n", we are only interested in the first token of answer -> idx 2
